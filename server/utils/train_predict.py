...
import server.utils.extract as extract
import server.utils.categorize as categorize
...
def graphology_prediction(file_name):

	if os.path.isfile("server/utils/label_list"):
		print("Info: label_list found.")
		#=================================================================
		with open("server/utils/label_list", "r") as labels:
			for line in labels:
				content = line.split()
		...
		baseline_angle, baseline_angle_comment = categorize.determine_baseline_angle(raw_baseline_angle)
		# print("Baseline Angle: "+baseline_angle_comment)

		raw_top_margin = raw_features[1]
		top_margin, top_margin_comment = categorize.determine_top_margin(raw_top_margin)
		# print("Top Margin: "+top_margin_comment)

		raw_letter_size = raw_features[2]
		letter_size, raw_letter_size_comment = categorize.determine_letter_size(raw_letter_size)
		# print("Letter Size: "+raw_letter_size_comment)

		raw_line_spacing = raw_features[3]
		line_spacing, raw_line_spacing_comment = categorize.determine_line_spacing(raw_line_spacing)
		# print("Line Spacing: "+raw_line_spacing_comment)

		raw_word_spacing = raw_features[4]
		word_spacing, raw_word_spacing_comment = categorize.determine_word_spacing(raw_word_spacing)
		# print("Word Spacing: "+raw_word_spacing_comment)

		raw_pen_pressure = raw_features[5]
		pen_pressure, pen_pressure_comment = categorize.determine_pen_pressure(raw_pen_pressure)
		# print("Pen Pressure: "+pen_pressure_comment)

		raw_slant_angle = raw_features[6]
		slant_angle, slant_angle_comment = categorize.determine_slant_angle(raw_slant_angle)
		# print("Slant: "+slant_angle_comment)
		...
		return {
			"baseline_angle":baseline_angle_comment,
			"top_margin":top_margin_comment,
			"letter_size":raw_letter_size_comment,
			"line_spacing": raw_line_spacing_comment,
			"word_spacing": raw_word_spacing_comment,
			"pen_pressure":pen_pressure_comment,
			"slant":slant_angle_comment,
			"emotional_stability": int(clf1.predict([[baseline_angle, slant_angle]])),
			"mental_energy_or_will_power": int(clf2.predict([[letter_size, pen_pressure]])),
			"modesty": int(clf3.predict([[letter_size, top_margin]])),
			"personal_harmony_and_flexibility": int(clf4.predict([[line_spacing, word_spacing]])),
			"lack_of_discipline": int(clf5.predict([[slant_angle, top_margin]])),
			"poor_concentration": int(clf6.predict([[letter_size, line_spacing]])),
			"non_communicativeness": int(clf7.predict([[letter_size, word_spacing]])),
			"social_isolation": int(clf8.predict([[line_spacing, word_spacing]])),
		}
		#=================================================================================================
		...
		return {
			"error":"label_list file not found."
		}
